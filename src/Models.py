...
class HFSelection():
    """ fusion strategy + hebbian laerning
    ...
    def sferes_call(self, sari, mean_rt, parameters):
        ...
        self.reaction = np.zeros(self.N)
        # Hebbian #####                
        self.values_hebbian = np.zeros((self.n_action*self.n_state,self.n_action))
        # Working memory ####
        self.p_a = np.zeros((int(self.parameters['length']), self.n_action))
        self.p_r_a = np.zeros((int(self.parameters['length']), self.n_action, 2))
        self.nb_inferences = 0
        ...
        for i in xrange(self.N):                    
            ...
            r = self.sari[i][0]            
            # SEARCH OR REPEAT
            self.state = self.sari[i][4]
            self.p_a_hebbian = SoftMaxValues(self.values_hebbian[self.last_action+self.n_action*self.state], self.parameters['gamma'])    
            self.Hf = -(self.p_a_hebbian*np.log2(self.p_a_hebbian)).sum()
            # BAYESIAN CALL
            self.p = self.uniform[:,:]
            self.Hb = self.max_entropy
            self.nb_inferences = 0
    ...
    def analysis_call(self, sari, mean_rt, parameters):
        ...
        for i in xrange(self.N):        
            ...
            self.entropy_list[i,0] = np.dot(self.p_decision, self.Hb_list[i])
            self.entropy_list[i,1] = self.Hf
            self.free_list[i] = self.p_a_hebbian
            tmp = np.dot(self.p_decision,q_values)            
            self.based_list[i] = tmp/tmp.sum()
            # self.biais_list[i] = self.spatial_biases            
            if self.n_element:
    ...
    def fusionModule(self):
        np.seterr(invalid='ignore')
        self.values_net = self.p_a_mb+self.values_hebbian[self.last_action+self.n_action*self.state]
        tmp = np.exp(self.values_net*float(self.parameters['beta']))
    ...
    def updateValue(self, reward):        
        # Updating hebbian learning
        line = self.last_action+self.n_action*self.state
        self.values_hebbian[line, self.current_action] += self.parameters['alpha']*(1.0 - self.values_hebbian[line, self.current_action])        
        index = range(self.n_action)
        index.pop(int(self.current_action))        
        self.values_hebbian[line,index] += self.parameters['alpha']*(0.0 - self.values_hebbian[line,index])
...
class MetaFSelection():
    ...
    def analysis_call(self, sari, mean_rt, parameters):
        ...
        for i in xrange(self.N):        
            if self.sari[i][4]-self.sari[i-1][4] < 0.0 and i > 0:                    
                    ...
                    nb_repeat = np.sum(self.sari[start:i-1,4] == 1)
                    if nb_repeat >= 3 and nb_search <= 5:
                        w = (self.max_entropy - self.entropy_list[start:start+nb_search+3,1])/(2.0*self.max_entropy - self.entropy_list[start:start+nb_search+3,0] - self.entropy_list[start:start+nb_search+3,1])
                        self.wmean_dict[nb_search][:] += 1.0 - w                        
                        self.wmean_count[nb_search][:] += 1.0
                    elif nb_repeat < 3 and nb_search <= 5:
                        w = (self.max_entropy - self.entropy_list[start:start+nb_search+nb_repeat,1])/(2.0*self.max_entropy - self.entropy_list[start:start+nb_search+nb_repeat,0] - self.entropy_list[start:start+nb_search+nb_repeat,1])
                        self.wmean_dict[nb_search][0:nb_search+nb_repeat] += 1.0 - w
                        self.wmean_count[nb_search][0:nb_search+nb_repeat] += 1.0
                    start = i
                    #######################                          
        ...
        ##########################
        for k in self.wmean_dict.iterkeys():
            tmp = self.wmean_dict[k]/self.wmean_count[k]            
            tmp[np.isnan(tmp)] = 0.0
            self.wmean_dict[k] = tmp            
        ##########################
    ...
    def sigmoideModule(self, m = 2.0):
        np.seterr(invalid='ignore')
        x = 2*self.max_entropy-self.Hb-self.Hf + 2.0 * self.Hmeta       
        # x = m-self.Hb-self.Hf        
...
class Sweeping():
    """ sweeping at the end of search trials
    same that fusion strategy
    """
    def __init__(self):
        #Parameters
        self.n_action = 4
    ...
    def analysis_call(self, sari, mean_rt, parameters):
        ...
        # for i in xrange(12):                                
            ...
            q_values = np.zeros((int(self.parameters['length'])+1, self.n_action))
            reaction = np.zeros(int(self.parameters['length'])+1)
            self.Hb_list = np.zeros((int(self.parameters['length'])+1))
            # NO SWEEPING
            if self.sari[i][4] == 1 or i == 0 or self.sari[i][4]-self.sari[i-1][4] < 0.0:
                self.p = self.uniform[:,:]                
                self.Hb = self.max_entropy
            ...
            # print self.p_decision
            # print self.Hb_list
            self.entropy_list[i,0] = np.dot(self.p_decision, self.Hb_list)
            self.entropy_list[i,1] = self.Hf
            self.free_list[i] = self.p_a_mf
            tmp = np.dot(self.p_decision,q_values)            

            self.based_list[i] = tmp
            self.biais_list[i] = self.spatial_biases            
            if self.n_element:
                self.inference_list[i] = np.dot(self.p_decision, np.arange(int(self.parameters['length'])+1))
            else:
